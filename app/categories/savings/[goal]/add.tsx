...
import { addDeposit, useGoals } from '@/lib/categoryStore';
...
export default function AddSavingsScreen() {
    ...
    const initialGoal = goals.find((g) => g.name.toLowerCase() === (params.goal ?? '').toLowerCase())?.name ?? goals[0]?.name ?? '';
    ...
    const goBack = () => router.replace({ pathname: '/categories/[name]', params: { ...params, name: params.goal ?? goal } });

    const save = () => {
        const parsedAmount = Number.parseFloat(amount);
        if (!Number.isFinite(parsedAmount) || parsedAmount <= 0 || !goal) return;
        addDeposit({
            goal,
            date: formatIso(date),
            title: title.trim() || `${goal} Deposit`,
            amount: parsedAmount,
            message: message.trim() || undefined,
        });
        router.replace({ pathname: '/categories/[name]', params: { ...params, name: goal } });
    };
        ...
        <SafeAreaView style={styles.safeArea}>
            <ScrollView contentContainerStyle={styles.content}>
                ...
                <View style={styles.panel}>
                    ...
                    <Pressable style={styles.selectField} onPress={() => { setDateOpen((open) => !open); setGoalOpen(false); }} accessibilityLabel="Choose date">
                    ...
                    <Pressable style={styles.selectField} onPress={() => { setGoalOpen((open) => !open); setDateOpen(false); }} accessibilityLabel="Select goal">
                        <Text style={goal ? styles.selectValueBold : styles.selectPlaceholder}>{goal || 'Select the category'}</Text>
                        <Ionicons name={goalOpen ? 'chevron-up' : 'chevron-down'} size={16} color={palette.muted} />
                    </Pressable>
                    {goalOpen && (
                        <View style={styles.dropdown}>
                            {goals.map((option) => (
                                <Pressable key={option.name} style={styles.dropdownItem} onPress={() => { setGoal(option.name); setGoalOpen(false); }}>
                    ...
                    <TextInput
                        ...
                        placeholder={goal ? `${goal} Deposit` : 'Deposit title'}
                        placeholderTextColor={palette.muted}
                        style={styles.textInput}
                    />
                    ...
                    <Pressable style={styles.saveButton} onPress={save} accessibilityLabel="Save deposit">
